...
class TestReview(DefaultMixin):
    ...
    def test_decline_change_request(self, dbsession: Session, content_type, change_type):
        ...
        if content_type == ContentType.SCHEMA:
            obj = self.get_default_schema(dbsession)
        else:
            obj = self.get_default_entity(dbsession)
        if change_type != ChangeType.CREATE:
            change_request.object_id = obj.id
        v = ChangeValueStr()
    ...
    def make_change_for_review(self, db: Session) -> ChangeRequest:
        entity = self.get_default_entity(db)
    ...
    def test_review_changes(self, dbsession: Session):
        user = dbsession.execute(select(User)).scalar()
        change_request = self.make_change_for_review(db=dbsession)
    ...
    def test_get_pending_change_requests(self, dbsession: Session):
