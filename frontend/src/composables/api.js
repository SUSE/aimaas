...
class API {
  ...
  async _is_response_ok(response) {
    ...
    if (Array.isArray(detail.detail)) {
      throw new Error(
        detail.detail.map((d) => `${d.loc}: ${d.msg}`).join(", ")
      );
      ...
      throw new Error(
        "Failed to process request. See console for more details."
      );
    }
  }
  ...
  async reviewChanges({ changeId, verdict, comment = null } = {}) {
    ...
    if (response) {
      this.alerts.push(
        "success",
        `Change request ${changeId} successfully reviewed.`
      );
    }
    return response;
  }
  ...
  async updateGroup({ groupId, body }) {
    ...
    if (response !== null) {
      this.alerts.push(
        "success",
        `Changes to group have been saved: ${body.name}`
      );
    }
    return response;
  }
  ...
  async addMembers({ groupId, userIds }) {
    ...
    if (response !== null) {
      if (response) {
        ...
        this.alerts.push(
          "warning",
          "No new members added to group because requested users are already members."
        );
      }
    }
    return response;
  ...
  async removeMembers({ groupId, userIds }) {
    ...
    if (response !== null) {
      if (response) {
        ...
        this.alerts.push(
          "warning",
          "No members were removed from group because requested users were no members."
        );
      }
    }
    return response;
  ...
  async getPermissions({
    ...
    return await this._fetch({
      url: `${this.base}/permissions?${params.toString()}`,
    });
  }
